<script>
  ...
  const views = Object.freeze([
    { id: 'browse', number: '01', label: 'Catalog', description: 'Find cosmetics' },
    { id: 'collection', number: '02', label: 'Owned', description: 'Your cosmetics' }
  ]);
...
<aside class="shop-rail" aria-label="Shop workspace navigation">
  <div class="shop-rail__heading">
    ...
    <strong>{activeView === 'browse' ? 'Browse cosmetics' : 'Your cosmetics'}</strong>
  </div>
  ...
  {#if activeView === 'browse'}
    <div class="shop-rail__section-heading"><span>Catalog</span><strong>{activeSection === 'names' ? 'Name effects' : activeSection === 'featured' ? 'Featured cosmetics' : 'Browse cosmetics'}</strong></div>
    ...
    <div class="shop-rail__section-heading shop-rail__section-heading--rarity"><span>Rarity</span><strong>Filter cosmetics</strong></div>
    <nav class="shop-rail__section-nav shop-rail__rarity-nav" aria-label="Filter by rarity">
      {#each rarityFilters as rarity (rarity.id)}
        <button
          type="button"
          class:active={activeRarity === rarity.id}
          class="rarity-filter"
          aria-pressed={activeRarity === rarity.id}
          aria-label={`${rarity.label}: ${rarity.count} cosmetics`}
          style={`--shop-rarity:${rarity.id === 'all' ? 'var(--shop-accent)' : rarity.id === 'Common' ? '#cdd0d8' : rarity.id === 'Uncommon' ? '#8ed5b2' : rarity.id === 'Rare' ? '#8db8ff' : rarity.id === 'Epic' ? '#c5a2ff' : rarity.id === 'Anomaly' ? '#ffb45f' : '#ff7fb2'}`}
          on:click={() => chooseRarity(rarity.id)}
        >
          <span class="shop-rail__rarity-swatch" aria-hidden="true"></span>
          <span class="shop-rail__copy"><strong>{rarity.label}</strong></span>
          <span class="shop-rail__count">{rarity.count}</span>
        </button>
      {/each}
    </nav>
...
<style>
  ...
  .shop-rail__section-heading--rarity { margin-top:.15rem; padding-top:.75rem; border-top:1px solid var(--shop-line); }
  .shop-rail__rarity-nav { gap:.15rem; }
  .shop-rail__rarity-nav .rarity-filter { grid-template-columns:.55rem minmax(0,1fr) auto; gap:.6rem; padding-block:.5rem; }
  .shop-rail__rarity-nav .rarity-filter.active { border-color:color-mix(in srgb,var(--shop-rarity) 54%,var(--shop-line)); background:color-mix(in srgb,var(--shop-rarity) 10%,transparent); }
  .shop-rail__rarity-swatch { display:block; width:.48rem; height:.48rem; border-radius:50%; background:var(--shop-rarity); box-shadow:0 0 0 .18rem color-mix(in srgb,var(--shop-rarity) 13%,transparent); }
  .shop-rail__rarity-nav .rarity-filter.active .shop-rail__rarity-swatch { box-shadow:0 0 0 .2rem color-mix(in srgb,var(--shop-rarity) 22%,transparent), 0 0 .7rem color-mix(in srgb,var(--shop-rarity) 38%,transparent); }
