...
<style>
  .profile-cosmetics-layout { display:grid; grid-template-columns:1fr; gap:1.25rem; align-items:start; }
  ...
  :global(.profile-settings-page__inner) { container-type:inline-size; container-name:profile-settings; }
  @container profile-settings (min-width: 70rem) {
    .profile-cosmetics-layout { grid-template-columns:minmax(0,2fr) minmax(20rem,1fr); }
  }
</style>
