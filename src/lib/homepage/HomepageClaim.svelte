...
{:else}
  <form class="homepage-claim" on:submit|preventDefault={submitClaim} aria-label="Claim your profile URL">
    ...
    <small id={error ? `${inputId}-error` : `${inputId}-hint`} class:homepage-claim__error={error} aria-live="polite">
...
<style>
  .homepage-claim { display: grid; gap: 0.5rem; width: min(100%, 410px); }
  ...
  .homepage-claim__field {
    ...
    font: 500 0.9rem / 1 'Clash Display', sans-serif;
  ...
  .homepage-claim__field > span { white-space: nowrap; }
  ...
  .homepage-claim__field button { min-width: 92px; padding: 0 18px; border-left: 1px solid var(--homepage-border); }
  .homepage-claim__field button:hover,
  .homepage-claim__primary:hover { background: var(--homepage-accent); }
  .homepage-claim__field button:active,
  .homepage-claim__primary:active { transform: translateY(1px); }

  .homepage-claim small {
    color: var(--homepage-muted-2);
    font: 400 0.63rem / 1.35 'Inter', sans-serif;
  }

  .homepage-claim__error { color: #ff8791 !important; }
  ...
  .homepage-claim__field button:focus-visible { outline: 2px solid var(--homepage-accent); outline-offset: 3px; }

  @media (max-width: 460px) {
    .homepage-claim__field { padding-left: 12px; }
    .homepage-claim__field input { padding-inline: 5px; font-size: 0.9rem; }
    .homepage-claim__field button { min-width: 82px; padding-inline: 10px; }
  }
