<script>
  export let isAuthenticated = false;
  export let variant = 'site';
</script>

<footer class="site-footer" class:site-footer--studio={variant === 'studio'} class:site-footer--home={variant === 'home'} class:site-footer--home-compact={variant === 'home-compact'} data-site-chrome="footer">
  {#if variant === 'home' || variant === 'home-compact'}
    <div class="site-footer__identity">
      <span class="site-footer__name">chm.lol</span>
      <span class="site-footer__tagline">A public profile that changes every day.</span>
    </div>

    <nav class="site-footer__nav" aria-label="Footer navigation">
      <a href="/leaderboard">Leaderboard</a>
      <a href="/pricing">Pricing</a>
      <a href="/how-to-play">How to play</a>
      <a href="/privacy">Privacy</a>
      <a href="/terms">Terms</a>
      <a href="mailto:support@chromadie.com">Support</a>
    </nav>
  {:else}
    <div class="site-footer__identity">
      {#if variant === 'studio'}
        <a class="site-footer__brand" href="/" aria-label="chm.lol home">
          <img class="site-footer__brand-logo" src="/brand/am-mark-v1.webp" alt="" width="52" height="42" decoding="async" />
        </a>
      {:else}
        <span class="site-footer__name">chm.lol</span>
      {/if}
      <span class="site-footer__tagline">A public profile that changes every day.</span>
    </div>

    <nav class="site-footer__nav" aria-label="Footer navigation">
      <a href="/">Roll</a>
      <a href="/leaderboard">Leaderboard</a>
      {#if isAuthenticated}<a href="/progression">Progress</a>{/if}
      {#if isAuthenticated}<a href="/profile/settings">Customize</a>{/if}
      <a href="/pricing">Pricing</a>
      <a href="/how-to-play">How to Play</a>
      <a href="/privacy">Privacy</a>
      <a href="/terms">Terms</a>
      <a href="mailto:support@chromadie.com">Support</a>
      <a href="mailto:business@chromadie.com">Business</a>
    </nav>
  {/if}
</footer>

<style>
  .site-footer {
    --site-footer-border: var(--homepage-border, rgba(255, 255, 255, 0.11));
    --site-footer-muted: var(--homepage-muted, #95959f);
    --site-footer-ink: var(--homepage-text, #f5f5f7);
    position: relative;
    z-index: 1;
    display: flex;
    width: min(1160px, calc(100% - 48px));
    align-items: center;
    justify-content: space-between;
    gap: 28px;
    margin: auto auto 0;
    padding: 30px 0 38px;
    border-top: 1px solid var(--site-footer-border);
    color: var(--site-footer-muted);
    font: 400 0.79rem / 1.45 'Inter', sans-serif;
  }

  .site-footer__identity {
    display: flex;
    flex-wrap: wrap;
    align-items: baseline;
    gap: 12px 24px;
  }

  .site-footer__name {
    color: rgba(245,245,247,.74);
    font-weight: 600;
  }

  .site-footer__tagline { color: rgba(245,245,247,.52); }

  .site-footer--home,
  .site-footer--home-compact {
    width: min(1160px, calc(100% - 48px));
    padding-block: 24px 30px;
  }

  .site-footer--home .site-footer__nav,
  .site-footer--home-compact .site-footer__nav { gap: 8px 16px; }

  .site-footer__nav {
    display: flex;
    flex-wrap: wrap;
    justify-content: flex-end;
    gap: 10px 18px;
  }

  .site-footer__nav a {
    color: rgba(245,245,247,.55);
    text-decoration: none;
    transition: color 0.18s ease;
  }

  .site-footer__nav a:hover,
  .site-footer__nav a:focus-visible {
    color: var(--site-footer-ink);
  }

  @media (max-width: 780px) {
    .site-footer {
      width: calc(100% - 30px);
      align-items: flex-start;
      flex-direction: column;
    }

    .site-footer__nav { justify-content: flex-start; }

  }

  @media (max-height: 32rem) {
    .site-footer { padding-block: 20px 24px; }
  }

  @media (prefers-reduced-motion: reduce) {
    .site-footer__nav a { transition: none; }
  }
</style>
