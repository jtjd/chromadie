...
<section class="selection-panel" aria-live="polite">
  ...
  {:else}
    <div class="selection-panel__eyebrow">Selected cosmetic · {SHOP_SLOT_LABELS[item.slot] || item.slot}</div>
    <div class="selection-panel__heading">
      <div>
        <span>{item.collection || `${item.rarity} cosmetic`}</span>
        <h2>{item.name}</h2>
      </div>
      <span class="selection-panel__rarity">{item.rarity || 'Common'}</span>
    </div>
    ...
    <div class="selection-panel__actions">
      ...
      {/if}
      <button type="button" class="selection-panel__reset" on:click={() => dispatch('reset')}>Reset preview</button>
    </div>
...
<style>
  .selection-panel { padding:18px; border:1px solid rgba(255,255,255,.085); border-radius:20px; background:linear-gradient(160deg, rgba(18,18,25,.96), rgba(10,11,15,.98)); }
  .selection-panel__empty { display:grid; gap:7px; min-height:150px; align-content:center; }
  .selection-panel__empty > span, .selection-panel__eyebrow { color:#a99cf5; font:700 .58rem/1 var(--font-mono-stack); letter-spacing:.1em; text-transform:uppercase; }
  ...
  .selection-panel h2 { overflow:hidden; margin:5px 0 0; color:#f2f0f7; font:720 1.55rem/1 var(--font-display); letter-spacing:-.04em; text-overflow:ellipsis; white-space:nowrap; }
  .selection-panel__rarity { flex:0 0 auto; padding:6px 8px; border:1px solid rgba(255,255,255,.1); border-radius:999px; color:#ccc8dc; font-size:.56rem; text-transform:uppercase; }
  .selection-panel__description { margin:13px 0 16px; color:#999aa7; font-size:.76rem; line-height:1.5; }
  ...
  .selection-panel__meta strong { margin-top:4px; color:#e1dfea; font-size:.62rem; }
  .selection-panel__actions { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:8px; margin-top:18px; }
  .selection-panel__primary, .selection-panel__reset { display:flex; align-items:center; justify-content:center; min-height:46px; border-radius:12px; cursor:pointer; font:700 .66rem var(--font-display); text-align:center; text-decoration:none; }
  .selection-panel__primary { border:1px solid rgba(153,130,255,.3); background:rgba(132,108,255,.14); color:#e1dcff; }
  .selection-panel__primary:disabled { border-color:rgba(255,255,255,.07); background:rgba(255,255,255,.035); color:#717381; cursor:not-allowed; }
  .selection-panel__reset { padding:0 12px; border:1px solid rgba(255,255,255,.09); background:transparent; color:#aaa9b5; }
  ...
  .selection-panel__related-list span { display:block; overflow:hidden; color:#c9c6d4; font-size:.56rem; text-overflow:ellipsis; white-space:nowrap; }
