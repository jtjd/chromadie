...
<section class="selection-panel" aria-live="polite">
  ...
  {:else}
    ...
    <p id={`shop-product-description-${item.item_key}`} class="selection-panel__description">{item.description}</p>
...
<style>
  ...
  .selection-panel__confirmation { display:flex; justify-content:space-between; gap:10px; margin-top:9px; padding:10px 12px; border:1px solid rgba(153,130,255,.2); background:rgba(132,108,255,.07); color:#9795a4; font-size:.62rem; }
  ...
  .selection-panel__related-list :global(.shop-preview-area) { height:58px; margin:0 0 5px; padding:4px; border-radius:3px; }
  .selection-panel__related-list span { display:block; overflow:hidden; color:#c9c6d4; font-size:.76rem; text-overflow:ellipsis; white-space:nowrap; }
