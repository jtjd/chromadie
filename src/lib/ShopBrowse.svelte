...
<section class="shop-browse" aria-labelledby="shop-browse-title">
  ...
  <nav class="shop-browse-categories" aria-label="Browse categories">
    {#each availableSections as category (category.id)}
      <button type="button" class:active={section === category.id} on:click={() => dispatch('section', category.id)}>{category.id === 'overview' ? 'All' : category.label}</button>
    {/each}
  </nav>

  {#if subsections.length}
    <div class="shop-subfilters" aria-label="Catalog types">
  ...
  <div class="shop-browse-toolbar">
    ...
    <label><span class="shop-filter-label">Sort</span><select bind:value={sortMode}>{#each SHOP_SORTS as sort (sort.id)}<option value={sort.id}>{sort.label}</option>{/each}</select></label>
...
<style>
  .shop-browse { display:grid; gap:1.35rem; }
  ...
  .shop-browse-categories, .shop-subfilters { display:flex; flex-wrap:wrap; gap:.35rem; }
  .shop-browse-categories button, .shop-subfilters button { min-height:2.35rem; padding:0 .8rem; border:1px solid #3a3d46; border-radius:5px; background:#111319; color:#aaa8b0; cursor:pointer; }
  .shop-browse-categories button.active, .shop-browse-categories button:hover, .shop-browse-categories button:focus-visible, .shop-subfilters button.active, .shop-subfilters button:hover, .shop-subfilters button:focus-visible { border-color:#777d8d; background:#181b22; color:#fff; }
  .shop-subfilters { padding-bottom:.9rem; border-bottom:1px solid var(--shop-line); }
  .shop-subfilters button { min-height:2rem; padding:0 .65rem; border-color:transparent; background:transparent; font-size:.8rem; }
  .shop-browse-toolbar { display:grid; grid-template-columns:minmax(16rem,1fr) repeat(4,minmax(8.5rem, auto)) auto; gap:.65rem; align-items:end; padding-bottom:1rem; border-bottom:1px solid var(--shop-line); }
  ...
  .shop-filter-label { display:block; margin:0 0 .35rem .1rem; color:#858690; font:.65rem var(--font-mono-stack); letter-spacing:.08em; text-transform:uppercase; }
  .shop-browse-toolbar select { width:100%; min-height:2.7rem; padding:0 .65rem; border:1px solid #4a4d57; border-radius:5px; background:#121419; color:#d9d7d2; font-size:.82rem; }
  .shop-affordable { display:flex; align-items:center; gap:.45rem; min-height:2.7rem; color:#aaa8b0; font-size:.78rem; white-space:nowrap; }
  ...
  @media (max-width: 1150px) { .shop-browse-toolbar { grid-template-columns:minmax(14rem,1fr) repeat(3,minmax(8rem,1fr)); } .shop-browse-toolbar label:nth-of-type(5) { grid-column:span 1; } .shop-affordable { grid-column:span 1; } .shop-result-grid { grid-template-columns:repeat(3,minmax(0,1fr)); } }
  @media (max-width: 760px) { .shop-surface-heading { align-items:flex-start; flex-direction:column; } .shop-browse-toolbar { grid-template-columns:1fr 1fr; } .shop-search { grid-column:1 / -1; } .shop-result-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } }
  @media (max-width: 520px) { .shop-browse-toolbar { grid-template-columns:1fr; } .shop-search { grid-column:auto; } .shop-result-grid { grid-template-columns:1fr; } }
</style>
