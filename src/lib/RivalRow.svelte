...
<article class="rival-row" class:inaccessible={item.inaccessible} style={`--rival-accent:${accent};--rival-rarity:${rarity.color}`}>
  <span class="rival-row__mark" aria-hidden="true">
    {#if item.inaccessible}—{:else}<UserAvatarFallback initial={item.displayName} />{/if}
  </span>
