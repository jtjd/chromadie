...
const MILESTONE_MANIFEST = Object.freeze([
  Object.freeze({
    ...
    reward: Object.freeze({
      itemKey: 'name_motion_typewriter_name',
      name: 'Type In',
      slot: 'name_motion'
    })
  }),
  Object.freeze({
    ...
    reward: Object.freeze({
      itemKey: 'name_material_carbon_cut',
      name: 'Carbon Vein',
      slot: 'name_material'
    })
  }),
  Object.freeze({
    ...
    reward: Object.freeze({
      itemKey: 'name_motion_haunt_glow',
      name: 'Glow',
      slot: 'name_motion'
    })
  }),
  Object.freeze({
    ...
    reward: Object.freeze({
      itemKey: 'name_material_glass_emboss',
      name: 'Raised Glass',
      slot: 'name_material'
    })
  }),
  Object.freeze({
    ...
    reward: Object.freeze({
      itemKey: 'name_motion_letter_shuffle',
      name: 'Scramble',
      slot: 'name_motion'
    })
  })
...
function normalizeEp(value) {
  ...
  return Math.min(MAX_EP, Math.max(0, Math.trunc(value)));
}
...
function normalizeMilestoneEntry(value) {
  const manifest = MILESTONE_BY_ID.get(milestoneKey(value));
  if (!manifest) return null;

  const entry = isRecord(value) ? value : {};
  const unlockedAt = normalizeTimestamp(entry.unlocked_at || entry.unlockedAt);
  const unlocked = entry.unlocked === true || Boolean(unlockedAt);
...
function normalizeMilestoneList(entries) {
  ...
  return MILESTONE_MANIFEST
    .map(manifest => normalized.get(manifest.id) || {
      id: manifest.id,
      name: manifest.name,
      description: manifest.description,
      threshold: manifest.threshold,
...
export function normalizeProgressionData(value, fallbackEp = null) {
  ...
  const recentUnlocks = normalizeNewMilestones(recentSource)
    .map(milestone => ({ ...milestone, unlockedAt: normalizeTimestamp(
      isRecord(recentSource.find(entry => milestoneKey(entry) === milestone.id))
        ? recentSource.find(entry => milestoneKey(entry) === milestone.id).unlocked_at
        : null
    ) }))
    .slice(0, MAX_RECENT_UNLOCKS);
  ...
  return {
    ...
    milestones: normalizeMilestoneList(payload.milestones || payload.milestone_track),
    recentUnlocks
  };
}
