...
export async function onRequestGet({ request, env }) {
  ...
  <text x="92" y="248" fill="#c7c9d1" font-family="Satoshi,Arial,sans-serif" font-size="30">${challenge ? `${sender} challenged you` : 'Challenge unavailable'}</text>
  <text x="92" y="365" fill="#f1f2f5" font-family="Cabinet Grotesk,Arial,sans-serif" font-size="92" font-weight="700">${escapeXml(score)} EP</text>
  <text x="92" y="430" fill="#9b9eaa" font-family="Satoshi,Arial,sans-serif" font-size="26">Beat this score · ${escapeXml(hex.toUpperCase())}</text>
