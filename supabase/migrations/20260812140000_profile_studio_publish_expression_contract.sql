-- Return the same complete expression-aware V2 projection used by the
-- owner configuration read after a Studio publish. The structured draft and
-- the legacy expression columns are one client contract at this boundary.
...
BEGIN
  ...
  BEGIN
    ...
    -- This is deliberately the same projection as the normal owner read.
    -- It overlays avatar/background/audio and rich media columns onto both
    -- V2 envelopes before the client clears its draft/preview overlays.
    v_complete := public.get_my_profile_configuration_v2();
    IF coalesce((v_complete->>'success')::boolean, false) IS NOT TRUE THEN
      v_error_code := NULL;
      v_error_message := coalesce(v_complete->>'error', 'The published profile configuration could not be reloaded.');
      RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = v_error_message;
    END IF;
  ...
  RETURN jsonb_build_object(
    'success', true,
    'version', 2,
    'draft', v_complete->'draft',
    'published', v_complete->'published',
    'updated_at', v_complete->'updated_at',
    'published_at', v_complete->'published_at',
    'identity', v_identity
  );
END;
$function$;
...
COMMENT ON FUNCTION public.publish_profile_studio_v2(jsonb, text, text, timestamptz) IS
  'Atomic Profile Studio boundary returning the complete expression-aware V2 configuration projection.';
