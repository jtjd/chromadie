-- The dedicated progression route is a distinct, still aggregate-only
-- measurement surface. Keep the recorder dimension allow-list explicit.
BEGIN;

ALTER TABLE public.progression_analytics_daily
  DROP CONSTRAINT IF EXISTS progression_analytics_daily_surface_check;

ALTER TABLE public.progression_analytics_daily
  ADD CONSTRAINT progression_analytics_daily_surface_check CHECK (
    surface IN ('', 'studio', 'progression', 'dedicated-roll', 'root-roll', 'roll')
  );
