-- Discovery profile previews.
--
-- Keep the existing bounded ranking projection as the source of truth, then
-- add the small public identity slice needed to make each discovery result
-- feel like a person rather than an anonymous score row. No private ids,
-- account data, draft configuration, or economy fields are exposed.

BEGIN;

ALTER FUNCTION public.get_public_discovery(text, text, text, integer, integer)
  RENAME TO get_public_discovery_base;
...
AS $function$
  ...
  SELECT jsonb_set(payload, '{items}', items, true)
  FROM rebuilt;
$function$;

COMMENT ON FUNCTION public.get_public_discovery(text, text, text, integer, integer) IS
  'Bounded public discovery projection with a minimal profile preview: display name, bio, accent, and validated public avatar path.';

REVOKE ALL ON FUNCTION public.get_public_discovery_base(text, text, text, integer, integer) FROM PUBLIC, anon, authenticated;
