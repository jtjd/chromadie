-- Publish the fourth official profile composition while preserving all prior
-- layout keys and the server-side configuration boundary.
BEGIN;
...
BEGIN
  ...
  IF v_value IN ('compact', 'full-bleed', 'sleek', 'framed') THEN RETURN v_value; END IF;
  IF v_fallback IN ('compact', 'full-bleed', 'sleek', 'framed') THEN RETURN v_fallback; END IF;
  RETURN 'compact';
END;
$function$;
...
INSERT INTO public.shop_items (
  ...
  'profile_layout_sleek', 'Sleek', 'profile_layout', 0, 'renderer', 'sleek', NULL, NULL,
  'Uncommon', 'A restrained rounded profile card with a tighter identity footprint and overlapping avatar.',
...
ALTER TABLE public.shop_items ADD CONSTRAINT shop_items_shape_check CHECK (
  ...
  AND (
    ...
    OR (slot = 'profile_layout' AND css_value IN ('compact', 'full-bleed', 'sleek', 'framed'))
...
VALUES ('shop_version', '2026-08-30T12:00:00Z')
