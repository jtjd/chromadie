-- Include the owner's selected public avatar reference in the existing
-- authenticated bootstrap projection so account chrome can use the same
-- provider-neutral media contract as public profile surfaces.
-- No private media-library rows, storage credentials, or public identity
-- projections are exposed by this change.
...
BEGIN
  ...
  SELECT json_build_object(
      ...
      'avatar_reference', public.profile_media_public_reference(c.avatar_asset_id, c.avatar_path)
    )
  INTO v_profile
...
COMMENT ON FUNCTION public.get_my_profile() IS
  'Authenticated owner profile projection and missing-profile recovery, including the provider-neutral selected avatar reference used by account chrome.';
