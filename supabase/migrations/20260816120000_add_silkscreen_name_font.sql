-- Add the Silkscreen display face used by the approved pixel-profile reference.
-- This is additive: existing active and legacy rows, inventory, and equipped
-- profile references remain unchanged.
BEGIN;
...
INSERT INTO public.shop_items (
  ...
  'name_font_silkscreen', 'Silkscreen', 'name_font', 0, 'renderer', 'silkscreen', NULL, NULL,
  'Rare', 'A compact pixel display face with a crisp, arcade-like identity voice.',
  'Static Bloom', false, 'free', NULL, 'active'
)
...
VALUES ('shop_version', '2026-08-16T12:00:00Z')
...
BEGIN
  ...
  IF active_count <> 89 THEN RAISE EXCEPTION 'Expected 89 active catalog rows, found %', active_count; END IF;
  SELECT count(*) INTO font_count FROM public.shop_items WHERE slot = 'name_font' AND catalog_status = 'active';
  IF font_count <> 10 THEN RAISE EXCEPTION 'Expected 10 active Name Font rows, found %', font_count; END IF;
END;
$verification$;
