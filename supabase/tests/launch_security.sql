...
SELECT pg_temp.audit_assert(
  ...
  AND (SELECT count(*) = 9 FROM public.shop_items WHERE slot = 'profile_atmosphere' AND catalog_status = 'active')
  AND (SELECT count(*) = 123 FROM public.shop_items WHERE catalog_status = 'active')
  AND NOT EXISTS (
