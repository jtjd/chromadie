...
SELECT pg_temp.audit_assert(
  ...
  AND (SELECT count(*) = 40 FROM public.shop_items WHERE slot IN ('name_font', 'name_material', 'name_motion') AND catalog_status = 'active')
  ...
  AND (SELECT count(*) = 112 FROM public.shop_items WHERE catalog_status = 'active')
  AND NOT EXISTS (
