...
try {
  ...
  for (const width of [1440, 390, 320]) {
    ...
    const style = await page.evaluate(`(() => {
      ...
      const rarity=getComputedStyle(el.querySelector('.profile-roll-summary__rarity'));
      return {background:css.backgroundColor,shadow:css.boxShadow,color:css.color,parent:getComputedStyle(el.parentElement).color,swatch:swatch.backgroundColor,rarity:rarity.color,details:Boolean(el.querySelector('details')),overflow:document.documentElement.scrollWidth>innerWidth+1};
    })()`);
    ...
    assert.equal(style.rarity, 'rgb(110, 226, 164)');
    assert.equal(style.details, false);
    assert.equal(style.overflow, false);
    await page.screenshot(join(evidenceDir, `signature-${width}.png`));
  }
  console.log('PASS inherited appearance, canonical rarity color, exact swatch, reduced motion and desktop/mobile geometry');
} finally {
