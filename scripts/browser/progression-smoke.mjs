...
try {
  ...
  await step('established history and Premium fixture stay on server-owned boundaries', async () => {
    ...
    await chromium.page.waitFor("(() => { const result = [...document.querySelectorAll('[data-profile-widget]')].find(node => node.dataset.profileWidget === 'roll' && node.dataset.profileWidgetMode === 'summary'); const text = result?.textContent?.trim() || ''; const rarity = result?.querySelector('.profile-roll-summary__rarity')?.textContent?.trim() || ''; return Boolean(result && /#[0-9a-f]{6}/i.test(text) && /\\d[\\d,]*\\s*EP\\b/i.test(text) && rarity); })()", 'public daily-roll summary widget', 30000);
