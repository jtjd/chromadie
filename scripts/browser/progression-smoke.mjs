...
try {
  ...
  await step('established history and Premium fixture stay on server-owned boundaries', async () => {
    ...
    await chromium.page.waitFor("(() => { const result = [...document.querySelectorAll('[data-profile-widget]')].find(node => node.dataset.profileWidget === 'roll' && node.dataset.profileWidgetMode === 'summary'); const text = result?.textContent?.trim() || ''; const meta = result?.querySelector('.profile-roll-summary__meta')?.textContent?.trim() || ''; const details = result?.querySelector('.profile-roll-summary__details'); return Boolean(result && /#[0-9a-f]{6}/i.test(text) && /\\d[\\d,]*/.test(details?.textContent || '') && meta); })()", 'public daily-roll summary widget', 30000);
    ...
    Object.assign(publicState, await chromium.page.evaluate("(() => { const result = [...document.querySelectorAll('[data-profile-widget]')].find(node => node.dataset.profileWidget === 'roll' && node.dataset.profileWidgetMode === 'summary'); const meta = result?.querySelector('.profile-roll-summary__meta')?.textContent?.trim() || ''; const details = result?.querySelector('.profile-roll-summary__details'); return { rollResult: Boolean(result), rollIdentity: result?.querySelector('.profile-roll-summary__identity')?.textContent?.trim() || '', rollHex: meta, rollRarity: meta.replace(/#[0-9a-f]{6}/i, '').replace('·', '').trim(), rollScore: details?.textContent?.trim().replace(/\\s+/g, ' ') || '', rollDetails: Boolean(details) }; })()"));
    ...
    assert(publicState.rollResult && publicState.rollHex && publicState.rollRarity && publicState.rollDetails && /\d[\d,]*/.test(publicState.rollScore), 'Public profile did not render the canonical daily-roll summary widget: ' + JSON.stringify(publicState) + '.');
