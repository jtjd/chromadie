...
const rendererKeys = Object.freeze({
  ...
  profile_layout: new Set(['compact', 'sleek', 'minimal', 'modern', 'portfolio', 'full-bleed']),
...
const expectedCounts = Object.freeze({ name_font: 18, name_material: 7, name_motion: 11, profile_border: 9, cursor_trail: 16, avatar_effect: 18, profile_layout: 6, profile_atmosphere: 13, profile_motion: 1 });
...
if ([...seed.catalog.values()].filter(item => (item.catalog_status || 'active') === 'active').length !== 101) {
  fail(`expected 101 active catalog rows, found ${seed.catalog.size}`);
}
