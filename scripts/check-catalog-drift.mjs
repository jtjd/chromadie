...
const expectedCounts = Object.freeze({ name_font: 12, name_material: 8, name_motion: 15, profile_border: 11, cursor_trail: 23, avatar_effect: 15, profile_layout: 5, profile_atmosphere: 13, profile_motion: 3 });
...
if ([...seed.catalog.values()].filter(item => (item.catalog_status || 'active') === 'active').length !== 107) {
  fail(`expected 107 active catalog rows, found ${seed.catalog.size}`);
}
...
if (supabaseUrl && supabaseKey) {
  ...
  console.log(
    `Catalog drift check passed locally: final seed is valid (${seed.catalog.size} items; ` +
      `${expectedCounts.name_font} active Fonts, ${expectedCounts.name_material} active Materials, ${expectedCounts.name_motion} active Motions, ${expectedCounts.profile_border} active Profile Borders, ${expectedCounts.cursor_trail} active Cursor Trails, ${expectedCounts.avatar_effect} active Avatar Effects, ${expectedCounts.profile_layout} active structural Profile Layouts, ${expectedCounts.profile_atmosphere} active Atmospheres, ${expectedCounts.profile_motion} active Profile Motions; 107 active rows including non-renderer rows). ` +
