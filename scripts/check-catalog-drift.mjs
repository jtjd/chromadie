...
const cuteNameFontsMigration = await readFile(cuteNameFontsMigrationPath, 'utf8');
const authoredMotionMigrationPath = path.join(repoRoot, 'supabase/migrations/20260912120000_authored_name_motions.sql');
const authoredMotions = await readLocalCatalog(authoredMotionMigrationPath);
for (const [key, row] of authoredMotions.catalog) {
  const seeded = seed.catalog.get(key);
  if (!seeded || Object.keys(row).some(column => row[column] !== seeded[column])) {
    fail(`authored motion migration differs from seed for ${key}`);
  }
}
if (authoredMotions.catalog.size !== 5) fail('expected five new authored name motions');
...
const rendererKeys = Object.freeze({
  ...
  name_motion: new Set(['haunt-glow', 'letter-shuffle', 'typewriter-name', 'haunt-particles', 'haunt-rainbow', 'haunt-gradient', 'haunt-fuzzy', 'haunt-reveal', 'haunt-split', 'haunt-flash', 'kinetic-echo', 'magnetic-type', 'neon-particle', 'raster-signal', 'spectrum-flow', 'star-companions', 'heart-pop', 'ion-sweep', 'phase-fracture', 'letterpress']),
...
const expectedCounts = Object.freeze({ name_font: 17, name_material: 8, name_motion: 20, profile_border: 12, cursor_trail: 23, avatar_effect: 15, profile_layout: 5, profile_atmosphere: 14, profile_motion: 3 });
...
for (const requiredValue of [
  "'name_font_fredoka'",
  "'name_font_baloo_2'",
  "'name_font_bubblegum_sans'",
  "'name_font_comic_neue'",
  "'name_font_lilita_one'",
  "'fredoka'",
  "'baloo-2'",
  "'bubblegum-sans'",
  "'comic-neue'",
  "'lilita-one'",
  'Expected 112 active catalog rows',
  'Expected 17 active Name Font rows'
]) {
  if (!cuteNameFontsMigration.includes(requiredValue)) {
    fail(`the cute Name Font migration is missing ${requiredValue}`);
  }
}
...
if (supabaseUrl && supabaseKey) {
  ...
  console.log(
    `Catalog drift check passed locally: final seed is valid (${seed.catalog.size} items; ` +
      `${expectedCounts.name_font} active Fonts, ${expectedCounts.name_material} active Materials, ${expectedCounts.name_motion} active Motions, ${expectedCounts.profile_border} active Profile Borders, ${expectedCounts.cursor_trail} active Cursor Trails, ${expectedCounts.avatar_effect} active Avatar Effects, ${expectedCounts.profile_layout} active structural Profile Layouts, ${expectedCounts.profile_atmosphere} active Atmospheres, ${expectedCounts.profile_motion} active Profile Motions; ${[...seed.catalog.values()].filter(item => (item.catalog_status || 'active') === 'active').length} active rows including non-renderer rows). ` +
