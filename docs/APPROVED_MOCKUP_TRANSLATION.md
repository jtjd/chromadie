...
| Today’s color | `components/orb-roll.tsx` settled orb | `get_my_daily_roll`, `get_public_profile_scores`, and canonical `roll_die` response | `src/lib/Game.svelte` on `/roll` + static `src/lib/TodayColor.svelte` profile result | preserve existing |
| Decorative roll atmosphere | `components/orb-roll.tsx` candidate cycling | Decorative state only while the existing secure request is pending; canonical server result settles the UI | `src/lib/Game.svelte` | adapt |
| Next-roll state | `components/orb-roll.tsx` `todayColor.cooldown` | Existing UTC countdown in `Game.svelte`; no client eligibility inference | `src/lib/Game.svelte` + static `TodayColor.svelte` result | preserve existing |
