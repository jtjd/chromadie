...
| `src/lib/avatar-effect/AvatarEffect.svelte`, `AvatarParticles.svelte`, `public/avatar-effects/` | Shared avatar-local decoration wrapper; selected anchor effects layer authored SVG art and one bounded particle canvas | `IdentityCard.svelte`, discovery/leaderboard cards, shop/settings previews; decorative only, compact surfaces stay static |
