...
| --- | --- | --- |
| Shop catalog | `public.shop_items` plus `meta.shop_version`; local cache; `supabase/seed.sql` and additive catalog migrations; 99 active rows plus 42 retained legacy rows | `stores.js` validates Name, Border, Cursor, Avatar, paid Layout, and the 13 active Profile Atmosphere rows (including the structured Atelier Plus rows); `Shop.svelte` adds section/filter behavior and explicit access labels |
