...
test('the direct homepage specimen uses the canonical first-example layout without mounting the profile shell', async () => {
  ...
  assert.doesNotMatch(hero, /PREVIEW_ROLL|previewRoll|accentpreview|rollPhase|homepage-roll-particles|setLocalPreviewRoll|localLeaderboardEntry/);
  ...
  assert.match(demo, /latestRoll = fixture\?\.scores\?\.\[0\] \|\| null/);
  assert.doesNotMatch(demo, /previewRoll/);
...
test('the live community surface remains isolated from homepage fixtures', async () => {
  ...
  assert.match(community, /export let isAuthenticated = false/);
  assert.match(community, /export let username = ''/);
  assert.match(community, /CURRENT_USER_LOOKUP_LIMIT = 12/);
