...
test('rich media paths and quotas are exact and bounded', () => {
  ...
  assert.deepEqual(PROFILE_RICH_MEDIA_KINDS, ['background_video', 'animated_avatar', 'share_image', 'audio', 'cursor', 'pointer_cursor']);
  assert.equal(PROFILE_RICH_MEDIA_MAX_TOTAL_BYTES, 1024 * 1024 * 1024);
  ...
  assert.deepEqual(PROFILE_RICH_MEDIA_RULES.animated_avatar.extensions, ['gif', 'webp']);
  assert.equal(PROFILE_RICH_MEDIA_RULES.share_image.maxOutputBytes, 1024 * 1024);
...
test('rich media migration and renderer preserve ownership and browser safety boundaries', async () => {
  ...
  assert.match(editor, /select_my_profile_r2_media_v2/);
  ...
  assert.match(editor, /Active animated avatar/);
  assert.match(editor, /Active share preview/);
  assert.match(editor, /Active profile audio/);
  assert.match(editor, /Replace video/);
  assert.match(editor, /Replace preview/);
