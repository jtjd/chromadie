...
test('the dedicated Roll page preserves the authoritative Game surface inside the new shell', () => {
  ...
  assert.match(game, /One color\. Every day\./);
  ...
  assert.match(game, /No result yet/);
  assert.match(game, /Roll to generate today’s color/);
  ...
  assert.match(page, /Today’s roll is ready/);
  assert.match(page, /Roll once per day to generate a color, rarity, and score/);
  ...
  assert.match(reveal, /label: 'Channels'/);
