...
test('launch renderer registries contain exactly the requested finite keys', () => {
  ...
  assert.deepEqual(AVATAR_EFFECT_KEYS, ['3d-parallax', 'glitch-slicer', 'liquid-blob', 'cyber-hud', 'butterfly-orbit', 'fireflies', 'moonlit-clouds', 'enchanted-garden', 'prismatic-fracture', 'sakura-neko', 'cloud-bunny', 'crimson-ronin', 'midnight-oni', 'koi-current', 'sakura-petals', 'bat-orbit']);
  assert.equal(PROFILE_ATMOSPHERE_KEYS.length, 14);
  ...
  assert.equal(new Set(AVATAR_EFFECT_KEYS).size, 16);
...
test('reference avatar effects are code-owned, image-aware, and motion-safe', async () => {
  ...
  assert.deepEqual(Object.keys(AVATAR_EFFECT_DEFINITIONS), ['3d-parallax', 'glitch-slicer', 'liquid-blob', 'cyber-hud', 'butterfly-orbit', 'fireflies', 'moonlit-clouds', 'enchanted-garden', 'prismatic-fracture', 'sakura-neko', 'cloud-bunny', 'crimson-ronin', 'midnight-oni', 'koi-current', 'sakura-petals', 'bat-orbit']);
...
test('atmosphere scenes are finite, authored, and safe to mount repeatedly', async () => {
  ...
  assert.match(atmosphereSource, /AuthoredAtmosphereLayer/);
...
test('seed and migrations contain the launch products and version bumps', async () => {
  ...
  assert.equal((seed.match(/^\s+\('avatar_effect_[a-z0-9_]+'/gm) || []).length, 16);
  assert.equal((seed.match(/^\s+\('profile_layout_[a-z0-9_]+'/gm) || []).length, 5);
  assert.equal((seed.match(/^\s+\('profile_atmosphere_[a-z0-9_]+'/gm) || []).length, 14);
